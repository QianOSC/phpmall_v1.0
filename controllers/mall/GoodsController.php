...
class GoodsController extends MallController
{
    ...
    // 批量设置分类
    public function actionBatchUpdateCats()
    {
        $form = new GoodsForm();
        $form->attributes = \Yii::$app->request->post();
        $res = $form->batchUpdateCats();
    ...
    {
        $form = new GoodsListForm();
        $form->search = json_encode(\Yii::$app->request->post());
    ...
    public function actionOtherPermission()
    {
        if (\Yii::$app->request->isGet) {
            $form = new GoodsEditForm();
            $sign = \Yii::$app->request->get('plugin');
            return $this->asJson($form->getPermission($sign));
        }
    }
}
