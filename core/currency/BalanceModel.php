...
class BalanceModel extends Model implements BaseCurrency
{
    ...
    public function sendSmsToUser($type, $balance)
    {
        try {
            \Yii::warning('----余额变动消息发送提醒----');
            if (!$this->user->mobile) {
            ...
            $messageService->content = [
                'mch_id' => 0,
                'args' => [$type == 1 ? '增加' : '减少', $balance, price_format($this->user->userInfo->balance)]
            ];
            $messageService->platform = PlatformConfig::getInstance()->getPlatform($this->user);
            $messageService->tplKey = 'balance';
