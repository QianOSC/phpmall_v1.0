...
class PassportForm extends Model
{
    ...
    {
        return [
            [['username', 'password', 'user_type', 'checked'], 'required'],
...
//            [['pic_captcha'], 'captcha', 'captchaAction' => 'site/pic-captcha'],
    ...
    public function login()
    {
        ...
        try {
            ...
            if($user->identity->is_operator== 1)
                $this->user_type = 2;
            else if($user->identity->is_admin == 1)
                $this->user_type = 1;

            $is_supplier = false;
            if($user->identity->is_supplier == 1)
                $is_supplier = true;
