...
//            ->one();
    ...
    private function percentage()
    {
        ...
        $this->list['percentage'] = bcdiv(bcsub($user_count, $order_count + 1), $user_count, 2) ?? '0';
        return;
    }
    ...
    private function order_where()
    {
        ...
        try {
            ...
            $model->leftJoin(['vcd' => VipCardDiscount::tableName()], 'vcd.order_id = o.id')
                ->select(['sum(o.coupon_discount_price) as all_coupon_discount_price', 'sum(o.total_pay_price) as all_total_price',
                'sum(od.num) as all_num', '(sum(od.member_discount_price) + IFNULL(sum(vcd.discount),0)) as all_member_discount_price']);
        } catch (\Exception $exception) {
        ...
        try {
            $query = FootprintGoodsLog::find()->alias('fg')
                ...
                ->andWhere(['fg.mall_id' => \Yii::$app->mall->id, 'fg.user_id' => \Yii::$app->user->id, 'fg.is_delete' => 0,
                    'g.status' => 1])->andWhere(CommonGoodsList::showAuthCondition('g'));
            if ($this->start_time) {
...
//                }
                ...
                try {
                    if ($item['sign'] !== '') {
                        ...
                        if (method_exists($plugin, 'getIsNegotiable')) {
                            $item['is_negotiable'] = $plugin->getIsNegotiable($item);
                        }
                    }
                } catch (\Exception $exception) {
                    \Yii::error($exception);
