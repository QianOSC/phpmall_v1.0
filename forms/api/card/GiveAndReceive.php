...
class GiveAndReceive extends Model
{
    ...
    public function getStatus($userCard)
    {
        ...
        if ($userCard->order && ($userCard->order->cancel_status != 0 || ($userCard->detail->refund && $userCard->detail->refund != 3))) {
            $status = 4;
        }
        return $status;
    }
    ...
    public function poster()
    {
        ...
        try {
            ...
            /* @var UserCard $userCard */
            $userCard = $this->getUserCard();
            $qrcode = new CommonFunction();
            $s = new class extends ApiGrafika {
                public function __destruct()
                {
                    // do something
                }
            };
            $file_path = $qrcode->qrcode(['qr_code' => ['type' => 1]], [
                ['user_id' => \Yii::$app->user->id, 'card_id' => $this->cardId],
                240,
                'pages/card/give/give'
            ], $s);
            $file_path = \Yii::$app->request->hostInfo . \Yii::$app->request->baseUrl . '/temp' . substr($file_path,strrpos($file_path,'/'));
            $res = [
                'avatar' => $userCard->user->userInfo->avatar,
                'nickname' => $userCard->user->nickname,
                'name' => $userCard->name,
