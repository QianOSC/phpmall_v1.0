...
class AttrForm extends Model
{
    ...
    public function getAttr()
    {
        ...
        try {
            ...
            if (!$goodsAuth->checkBuyAuth($goods)) {
                throw new \Exception('您暂无权限购买商品');
            }
            if ($goodsAuth->checkFinishSell($goods)) {
                throw new Exception('商品已结束销售');
            }

            $form->goods = $goods;
            $mallGoods = CommonGoods::getCommon()->getMallGoods($goods->id);
            $form->setMember(!$mallGoods || $mallGoods->is_negotiable == 0);
            $form->setShare(false);
            ...
            $data = [
                ...
                'template_message_list' => $res['template_message_list']
            ];
            if ($res['level_show'] != 0) {
