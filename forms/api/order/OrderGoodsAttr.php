...
class OrderGoodsAttr extends Model
{
    ...
    {
        return [
            [['goods_id', 'stock', 'weight', 'id', 'goods_warehouse_id', 'use_integral', 'number', 'supplier_id'], 'integer'],
    ...
    public function setGoodsAttrById($goodsAttrId)
    {
        ...
        $goodsAttr = GoodsAttr::find()->with('share')->where(['id' => $goodsAttrId, 'goods_id' => $this->goods->id])->one();
        if (!$goodsAttr) {
