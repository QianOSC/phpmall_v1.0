...
class OrderSubmitForm extends Model
{
    ...
    public function getAllData()
    {
        ...
        foreach ($listData as &$mchItem) {
            ...
            if (isset($mchItem['next_full_reduce_discount'])) {
                $nextFullReduce = $mchItem['next_full_reduce_discount'];
            }
        ...
        foreach ($listData as &$item) {
            ...
            // 总折扣
            $item['total_discounts_price'] = price_format($item['total_goods_original_price'] - $item['total_price']);
        }
        ...
        if ($allZiti) {
            if (!$address) {
                /** @var Order $order */
                $order = Order::find()->where(['user_id' => $this->getUser()->id, 'send_type' => 1, 'is_delete' => 0])->orderBy(['created_at' => SORT_DESC])->one();
                if ($order) {
                    $address = [
                        'name' => $order->name,
                        'mobile' => $order->mobile
        ...
        return [
            'time'     => [
                't0'    => $mall['setting']['t0'],
                't1'    => $mall['setting']['t1'],
                't0_today'    => $mall['setting']['t0_today'],
                't1_today'    => $mall['setting']['t1_today'],
                't0_limit'    => $mall['setting']['t0_limit'],
                't1_limit'    => $mall['setting']['t1_limit'],
                't0_today_limit'    => $mall['setting']['t0_today_limit'],
                't1_today_limit'    => $mall['setting']['t1_today_limit'],
                'time_tips'    => $mall['setting']['time_tips'],
                ],
            'mch_list' => $listData,
            ...
            'delivery_time' => [
                'status'    => !empty($this->form_data['time']) ? $this->form_data['time']['listStatus'] : -1,
                'time'      => !empty($this->form_data['time']) ? $this->form_data['time']['time'] : '',
            ]
        ];
    }
    ...
    protected function getGoodsListData($goodsList)
    {
        ...
        foreach ($goodsList as $i => $item) {
            ...
            $this->goods_num_info[$item['id']] = isset($this->goods_num_info[$item['id']]) ? $this->goods_num_info[$item['id']] + $item['num'] : $item['num'];
            $list[] = $result;
        }
        return $list;
    }
    ...
    protected function setMemberDiscountData($mchItem)
    {
        ...
        $identity = UserIdentity::find()->andWhere(['is_delete' => 0, 'user_id' => $user->id])->one();
        if (!$identity) {
            return $mchItem;
        }
        ...
        if (!$member) {
            ...
            foreach ($mchItem['goods_list'] as &$goodsItem) {
                ...
                try {
                    $goodsMemberPrice = $this->getGoodsAttrMemberPrice($goodsAttr, 1, $goodsItem['num']);
                } catch (\Exception $e) {
        ...
        foreach ($mchItem['goods_list'] as &$goodsItem) {
            ...
            $goodsAttr = $goodsItem['goods_attr'];
            try {
                $goodsMemberPrice = $this->getGoodsAttrMemberPrice($goodsAttr, $member->level, $goodsItem['num']);
            } catch (\Exception $e) {
                throw new OrderException($e->getMessage());
            }
            if ($goodsMemberPrice && $goodsItem['is_level_alone'] == 1) {
    ...
    public function setFullReduceDiscountData($mchItem)
    {
        ...
        $strategy = new FullReduceStrategy($activity);
        $sub = $strategy->get($mchItem, $totalGoodsOriginalPrice, $totalGoodsPrice);
        $mchItem['next_full_reduce_discount'] = $strategy->getNext($mchItem, $totalGoodsOriginalPrice, $totalGoodsPrice);
        if ($sub == 0) {
            return $mchItem;
        }
    ...
    {
        $res = null;
        foreach ($mchItem['goods_list'] as $mchGoods) {
            /** @var Goods $goods */
            $goods = $mchGoods['goods_attr']->goods;
            $goodsSendType = GoodsAuth::create($goods->sign)->getSendType($goods, $sendType);
            if ($res === null) {
                $res = $goodsSendType;
            } else {
                $res = array_intersect($res, $goodsSendType);
            }
        }
        if (empty($res)) {
            throw new OrderException('无效的发货方式');
        }
        return $res;
    }
    ...
    protected function setDeliveryData($mchItem, $formMchItem)
    {
        ...
        $sendType = $this->getGoodsSendType($mchItem, $sendType);
        $sendType = $this->getNewSendType($sendType);
        if (!isset($formMchItem['send_type'])
            || $formMchItem['send_type'] === ''
            || $formMchItem['send_type'] === null) {
            $formMchItem['send_type'] = isset($this->form_data['send_type']) && $this->form_data['send_type'] ? $this->form_data['send_type'] : $sendType[0];
        }
        \Yii::warning($sendType);
        if (!in_array($formMchItem['send_type'], $sendType)) {
            if (in_array(\Yii::$app->appPlatform, ['ttapp']) && $formMchItem['send_type'] == 'city') {
                throw new OrderException('客户端不支持同城配送');
            }
    ...
    public function getUsableCouponList($mchData)
    {
        ...
        foreach ($allList as &$userCoupon) {
            ...
            if ($userCoupon->coupon_min_price > $goodsTotalOriginalPrice || $userCoupon->start_time > $nowDateTime) {
    ...
    public function extraGoodsDetail($order, $goodsItem)
    {
        ...
        $orderDetail->erase_price = isset($goodsItem['erase_price']) ? $goodsItem['erase_price'] : 0;
        $goodsInfo = [
        ...
        $orderDetail->back_price = isset($goodsItem['back_price']) ? $goodsItem['back_price'] : 0;
    ...
    {
        if (in_array($goods->sign, ['miaosha', 'flash_sale', 'wholesale'])) {
    ...
    protected function checkGoodsBuyLimit($goodsList)
    {
        ...
        foreach ($goodsIdMap as $goodsId => $item) {
            ...
            $limitBuyClass = LimitBuy::create($goods->sign, ['user_id' => $this->getUser()->id]);
            $limitBuyClass->goods = $goods;
            if (!$limitBuyClass->checkLimitBuy($item['num']) || $this->getIsMinNumber() && !$limitBuyClass->checkMinNumber($item['num'])) {
                throw new OrderException($limitBuyClass->errorMsg);
            }
        }
    }
    ...
    public function setDiscountDataAfterMember($mchItem, $formMchItem)
    {
        ...
        foreach ($mchItem['goods_list'] as &$goodsItem) {
            ...
            if (isset($goodsItem['sign']) && in_array($goodsItem['sign'], ['wholesale'])) {
                list($goodsItem, $mchItem) = \Yii::$app->plugin->getPlugin($goodsItem['sign'])->pluginDiscount($goodsItem, $mchItem, $this->goods_num_info);
                $hasWholesale = true;
            }
        }

        if ($hasFlashSale) {
            $mchItem = \Yii::$app->plugin->getPlugin('flash_sale')->pluginDiscountData($mchItem);
        }
        if ($hasWholesale) {
            $mchItem = \Yii::$app->plugin->getPlugin('wholesale')->pluginDiscountData($mchItem);
        }
        return $mchItem;
    }
    ...
    {
        if (isset($goodsItem['sign']) && in_array($goodsItem['sign'], ['miaosha', 'flash_sale', 'wholesale'])) {
    ...
    private function changeSign($mchItem)
    {
        ...
        if (in_array('miaosha', $signArray)) {
            ...
            $sign = 'flash_sale';
        } elseif (in_array('wholesale', $signArray)) {
            $sign = 'wholesale';
        } else {
            $sign = $this->sign;
        }
    ...
    {
        return ['', 'mch', 'miaosha', 'flash_sale', 'wholesale'];
    }
    ...
    public function checkBuyAuth($goodsList)
    {
        foreach ($goodsList as $item) {
            /** @var Goods $goods */
            $goods = $item['goods_attr']['goods'];
            if (!GoodsAuth::create($goods->sign, ['user' => $this->getUser()])->checkBuyAuth($goods)) {
                throw new OrderException('您暂无权限购买商品(' . $goods->goodsWarehouse->name . ')');
            }
        }
        return true;
    }
    ...
    public function checkTime($goodsList)
    {
        foreach ($goodsList as $item) {
            /** @var Goods $goods */
            $goods = $item['goods_attr']['goods'];
            if (!GoodsAuth::create($goods->sign)->checkTime($goods)) {
                throw new OrderException('商品(' . $goods->goodsWarehouse->name . ')暂未开售或已下架');
            }
        }
        return true;
    }
