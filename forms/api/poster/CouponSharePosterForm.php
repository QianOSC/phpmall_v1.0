...
class CouponSharePosterForm extends GrafikaOption implements BasePoster
{
    ...
    public function get()
    {
        ...
        {
            $n = imagettfbbox($option['nickname']['font'], 0, $this->font_path, $option['nickname']['text']);
            $n_width = $n[2] - $n[0];
...

        {
            if ($info['type'] == 1) {
                ...
                $p = imagettfbbox($option['price']['font'], 0, $this->font_path, $option['price']['text']);
                $p_width = $p[2] - $p[1];
        ...
        {
            $t = imagettfbbox($option['time']['font'], 0, $this->font_path, $option['time']['text']);
            $t_width = $t[2] - $t[0];
