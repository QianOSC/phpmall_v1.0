...
class CommonQrCode extends Model
{
    ...
    public function getQrCode($scene = [], $width = 430, $page = 'pages/index/index')
    {
        try {
            ...
            } elseif ($appPlatform == APP_PLATFORM_MOBILE) {
                $qrcodeServe = new QrcodeServe();
                $filePath = $qrcodeServe->getQrcode($appPlatform, [
                    'scene' => $scene,
                    'width' => $width,
                    'page' => $page
                ]);
            ...
            } elseif ($appPlatform == APP_PLATFORM_WECHAT) {
                $qrcodeServe = new QrcodeServe();
                $filePath = $qrcodeServe->getQrcode($appPlatform, [
                    'scene' => $scene,
                    'width' => $width,
    ...
    private function saveTempImageByContent($content)
    {
        ...
        //部分 http
        $url = \Yii::$app->request->hostInfo . \Yii::$app->request->baseUrl . '/temp/' . $imgName;
    ...
    public function getGeneralQrcode($args = [])
    {
        ...
        $parameter = $this->saveQrCodeParameter($token, $args['scene'], $args['page']);

        return [
            'file_path' => $webUri . $imgName,
            'created_at' => $parameter->created_at
        ];
    }
}
