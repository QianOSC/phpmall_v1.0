...
class BaseCityServiceList extends BaseCityService
{
    ...
    private function transformData($item)
    {
        ...
        $newItem['is_debug'] = isset($data['is_debug']) ? (int)$data['is_debug'] : 0;

        // 值有int 和 string 要区分
        if (isset($data['outer_order_source_desc'])) {
            if (is_numeric($data['outer_order_source_desc'])) {
                $newItem['outer_order_source_desc'] = (int)$data['outer_order_source_desc'];
            } else {
                $newItem['outer_order_source_desc'] = $data['outer_order_source_desc'];
            }
        }

        $newItem['delivery_service_code'] = isset($data['delivery_service_code']) ? (int)$data['delivery_service_code'] : '';
