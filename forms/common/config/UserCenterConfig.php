...
class UserCenterConfig extends Model
{
    ...
    public function getData($model)
    {
        ...
        if (!$model) {
            return $userCenterDefault;
        } else {
            $option = \Yii::$app->serializer->decode($model->config);
            return $this->checkDefault($option, $userCenterDefault);
        }
    }
    ...
    public function orderBar($option)
    {
        ...
        foreach ($option['order_bar'] as $k => $item) {
            $id = ((int)$k + 1);
            $id = $id == 3 ? 4: $id;
            $item['link_url'] = '/pages/order/index/index?status=' . $id;
            if ((int)$k + 1 === 4) {
                $item['link_url'] = '/pages/order/refund/index';

                $item['text'] = $orderInfoCount[$k+1] ?: '';
                $item['num'] = $orderInfoCount[$k+1] ? intval($orderInfoCount[$k+1]) : 0;
            }else{
