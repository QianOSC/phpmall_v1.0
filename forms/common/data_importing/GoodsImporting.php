...
class GoodsImporting extends BaseImporting
{
    public function import()
    {
        try {
            foreach ($this->v3Data as $datum) {
                ...
                $goodsWarehouse->cost_price = $datum['cost_price'];
                ...
                $goods->price = $datum['price'];
                $goods->use_attr = $datum['use_attr'];
                $goods->attr_groups = \Yii::$app->serializer->encode($newAttrGroups);
                $goods->goods_stock = $datum['goods_num'];
                $goods->virtual_sales = $datum['virtual_sales'] ? $datum['virtual_sales'] : 0;
                $goods->confine_count = $datum['confine_count'];
                $goods->pieces = 0;
                $goods->forehead = 0;
                $goods->freight_id = 0;
//                $goods->give_integral = '';
//                $goods->give_integral_type = '';
                $goods->forehead_integral = isset($integral['forehead']) && $integral['forehead'] ? (int)$integral['forehead'] : 0;
                ...
                $mallGoods->is_quick_shop= isset($datum['quick_purchase']) ? $datum['quick_purchase'] : 0;
                $mallGoods->is_sell_well= $datum['hot_cakes'] ?: 0;
                $mallGoods->is_negotiable= $datum['is_negotiable'] ?: 0;
                ...
                foreach ($attr as $aItem) {
                    ...
                    $goodsAttr->price = $aItem['price'] ?: $datum['price'];
                ...
                // 商品分类
                foreach ($datum['gc'] as $cat) {
                    if (isset(CatImporting::$catIds[$cat['cat_id']])) {
                        $goodsCatRelation = new GoodsCatRelation();
                        $goodsCatRelation->goods_warehouse_id = $goodsWarehouse->id;
                        $goodsCatRelation->cat_id = CatImporting::$catIds[$cat['cat_id']];
