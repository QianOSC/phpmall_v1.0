...
class IntegralGoodsImporting extends BaseImporting
{
    public function import()
    {
        try {
            foreach ($this->v3Data as $datum) {
                ...
                $goodsPicList = \Yii::$app->serializer->decode($datum['goods_pic_list']);
                $newGoodsPicList = [];
                foreach ($goodsPicList as $item) {
                    $arr = [];
                    $arr['id'] = 0;
                    $arr['pic_url'] = $item;
                    $newGoodsPicList[] = $arr;
                ...
                $goodsWarehouse->pic_url = \Yii::$app->serializer->encode($newGoodsPicList);
                $goodsWarehouse->video_url = '';
...
//                }
                ...
                $goods->goods_stock = 0;
                $goods->virtual_sales = $datum['virtual_sales'];
...
//                $goods->give_integral_type = '';
                ...
                // 商品分类
                if (isset(IntegralCatImporting::$integralCatIds[$datum['cat_id']])) {
                    $goodsCatRelation = new GoodsCatRelation();
                    $goodsCatRelation->goods_warehouse_id = $goodsWarehouse->id;
                    $goodsCatRelation->cat_id = IntegralCatImporting::$integralCatIds[$datum['cat_id']];
