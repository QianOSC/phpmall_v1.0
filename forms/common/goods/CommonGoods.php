...
class CommonGoods extends Model
{
    ...
    public function getGoods($id, $asArray = false)
    {
        ...
        $query = Goods::find()->with(['attr.share', 'attr.memberPrice', 'attr.shareLevel', 'goodsWarehouse.cats'])
            ...
            ->where(['id' => $id, 'mall_id' => \Yii::$app->mall->id, 'is_delete' => 0]);
    ...
    public function getGoodsDetail($id, $hasGoodsWarehouse = true)
    {
        ...
        $detail['param_content'] = \yii\helpers\BaseJson::decode($detail['param_content'], true) ?: [];
        $detail['send_type'] = $detail['send_type'] ? explode(',', $detail['send_type']) : [];
        if ($goods->is_time == 1) {
            $detail['sell_time'] = [$goods->sell_begin_time, $goods->sell_end_time];
        }
        return $detail;
    }
    ...
    public function transformAttr($goods)
    {
        ...
        foreach ($goods->attr as $key => $attrItem) {
            ...
            if ($goods->use_attr == 0) {
                ...
                $detail['bar_code'] = $attrItem['bar_code'];
            } else {
                // 多规格商品 总库存是根据规格库存相加
                $goodsNum += $attrItem['stock'];
            }
