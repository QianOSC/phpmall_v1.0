...
class CommonGoodsDetail extends Model
{
    ...
    protected $isLimitBuy = false; // 是否开启限购
    protected $isTemplateList = true; // 是否需要订阅消息
...
//$pickup = substr($pickup, -1) == ',' ? substr($pickup, 0, -1) : $pickup;
        ...
        if ($this->isShipping) {
            ...
            if (in_array('vip_card', $permission)) {
                ...
                if ($plugin) {
                    $userId = $this->user ? $this->user->id : \Yii::$app->user->id;
...
/* @var GoodsCardRelation $item */
    ...
    {
        if ($this->mch_id > 0) {
            array_push($ignore, 'goods_coupon_center');
        }
        ...
        // 商品分销价格关闭
        $this->mallSetting = $this->mall->getMallSetting([
            'is_share_price', 'is_common_user_member_price', 'is_member_user_member_price', 'is_sales', 'is_express'
        ]);
        $mallSetting = $this->mallSetting;
        ...
        $result['param_content'] = \yii\helpers\BaseJson::decode($this->goods->param_content) ?: [];
        $result['is_video_number'] = $this->isVideoNumber();
        $result['limit_buy'] = $this->getLimitBuy();
        $goodsAuth = GoodsAuth::create($this->goods->sign, ['user' => $this->user]);
        $result['buy_goods_auth'] = $goodsAuth->checkBuyAuth($this->goods);
        $result['sell_time'] = $goodsAuth->getSellTime($this->goods);
        $result['is_finish_sell'] = $goodsAuth->checkFinishSell($this->goods);
        $result['template_message_list'] = $this->getTemplateMessageList();
        unset($result['limit_buy_status']);
        unset($result['limit_buy_type']);
        unset($result['limit_buy_value']);
        unset($result['confine_count']);
        unset($result['sell_begin_time']);
        unset($result['sell_end_time']);
        return $result;
    }
    ...
    public function getTemplateMessageList()
    {
        ...
        return TemplateList::getInstance()->getTemplate(
            \Yii::$app->appPlatform,
            ['pay_advance_balance']
        );
    }
    ...
    public function isVideoNumber()
    {
        ...
        if ($this->user) {
            $isVideoNumber = $data['is_video_number'] ? true : false;

            if ($data['is_video_number_member']) {

                $levelList = [];
                foreach ($data['video_number_member_list'] as $member) {
                    $levelList[] = $member['level'];
                }

                if (!in_array($this->user->identity->member_level, $levelList)) {
                    $isVideoNumber = false;
                }
            }
        }
...
/* @var GoodsAttr[] $goodsAttr */
    ...
    /**
     * @param integer $shareType 分销类型
     * @param float $first 分销金额
     * @param float $price 商品金额
     * @return float
     * 分销佣金计算
     */
    private function shareMoney($shareType, $first, $price = null)
    {
        // 分销佣金是百分比还是固定金额
        if ($shareType == 0) {
            ...
            if ($price === null) {
                $price = $this->getPriceMax();
            }
            $share = price_format($price * $first / 100, 'float', 2);
        }
        return $share;
    }
        ...
        if ($this->goods->individual_share == 1) {
            ...
            if ($this->goods->attr_setting_type == 1) {
                foreach ($this->goods->attr as $item) {
                    if ($item->share) {
                        $first = max($first, $this->shareMoney(
                            $this->goods->share_type,
                            $item->share->share_commission_first,
                            $item->price
                        ));
                    }
                }
                ...
                foreach ($this->goods->share as $item) {
                    if ($item->goods_attr_id == 0) {
                        $first = $this->shareMoney($this->goods->share_type, $item->share_commission_first);
                        break;
                    }
                }
            }
            ...
            $first = $this->shareMoney($shareType, $shareSetting[ShareSetting::FIRST]);
        }
        return [
            'first' => $first,
            'shareType' => $shareType,
        ...
        if ($this->goods->individual_share == 1) {
            ...
            /* @var GoodsShare[] $goodsShareLevel */
                ...
                foreach ($goodsShareLevel as $item) {
                    $goodsShareLevelList[$item->goods_attr_id] = $item->share_commission_first;
                }
                foreach ($this->goods->attr as $item) {
                    if (isset($goodsShareLevelList[$item->id])) {
                        $first = max($first, $this->shareMoney(
                            $this->goods->share_type,
                            $goodsShareLevelList[$item->id],
                            $item->price
                        ));
                    }
                }
            ...
            /* @var GoodsShare $goodsShareLevel */
                ...
                $first = $this->shareMoney($this->goods->share_type, $goodsShareLevel->share_commission_first);
            }
            $shareType = $this->goods->share_type;
        } else {
        // 多商户没有全局分销价
            ...
            $first = $this->shareMoney($shareType, $shareLevel->first);
        }
        return [
            'first' => $first,
            'shareType' => $shareType,
    ...
    public function getLevelShow()
    {
        if (!$this->isMember) {
            ...
            if ($this->user && $this->user->identity->member_level) {
                $isMemberPrice = $this->mallSetting['is_member_user_member_price'] ? 1 : 0;
            } else {
                $isMemberPrice = $this->mallSetting['is_common_user_member_price'] ? 1 : 0;
            }
            if ($isMemberPrice == 0) {
                $levelShow = 0;
            }
    ...
    public function setIsLimitBuy($val)
    {
        $this->isLimitBuy = $val;
    }
    ...
    public function getLimitBuy()
    {
        ...
        $limitBuy = LimitBuy::create($this->goods->sign);
        $limitBuy->goods = $this->goods;
        return $limitBuy->getLimitBuy();
    }

    public function setIsTemplateList($val)
    {
        $this->isTemplateList = $val;
    }
}
