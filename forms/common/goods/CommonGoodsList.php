...
class CommonGoodsList extends Model
{
    ...
    public function rules()
    {
        return [
            ...
            [['is_array', 'isSignCondition', 'is_show'], 'default', 'value' => 0],
    ...
    public function getGoodsData($goods)
    {
        ...
        $apiGoods->user = $this->user;
        $apiGoods->deleteAttr = $this->deleteAttr;
    ...
    public function setShowAuth()
    {
        if ($this->is_show != 1) {
            return $this;
        }
        $condition = self::showAuthCondition('g');
        $this->query->andWhere($condition);
        return $this;
    }

    public static function showAuthCondition($alias = '')
    {
        $level = 0;
        ...
        $levelArr = [-1, $level];
        $globalAuth = \Yii::$app->mall->getMallSetting(['show_goods_auth']);
        $globalAuthArr = explode(',', $globalAuth['show_goods_auth']);
        $show = !empty(array_intersect($levelArr, $globalAuthArr));
        $alias = $alias ? $alias . '.' : '';
        return [
            'or',
            [
                'and',
                $alias . '`is_setting_show_and_buy_auth` = 0',
                $show ? '1=1' : '1=0'
            ],
            [
                'and',
                $alias . '`is_setting_show_and_buy_auth` = 1',
                [
                    'or',
                    'find_in_set(\'-1\',' . $alias . '`show_goods_auth`)',
                    'find_in_set(\'' . $level . '\',' . $alias . '`show_goods_auth`)'
                ]
            ]
        ];
    }
