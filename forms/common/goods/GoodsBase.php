...
class GoodsBase extends Model
{
    ...
    public function rules()
    {
        return [
            [['id', 'status', 'goods_id', 'sort', 'freight_id', 'cat_id',
                'give_integral_type', 'forehead_integral_type',
            ...
            [['batch_ids', 'cat_ids'], 'safe'],
            [['forehead_integral'], 'number', 'min' => 0],
            [['forehead_integral'], 'number', 'max' => 99999999],
            [['give_integral'], 'number', 'min' => 0],
            [['give_integral'], 'number', 'max' => 99999999],
        ];
    }
    ...
    public function switchStatus()
    {
        try {
            ...
            $goodsAuth = GoodsAuth::create($goods->sign);
            if ($goods->status == 1 && $goodsAuth->checkFinishSell($goods)) {
                throw new \Exception('商品销售时间结束，无法上架');
            }
    ...
    {
        try {
            if ($this->sort > 999999999) {
                throw new \Exception('排序值不能大于99999999');
            }
    ...
    public function batchUpdateCats()
    {
        if (!$this->validate()) {
            return $this->getErrorResponse();
        }
        ...
        try {
            if (empty($this->cat_ids) || empty($this->batch_ids)) {
                throw new \Exception('请选择分类');
            }

            $goodsWarehouseIds = Goods::find()->select('goods_warehouse_id')->where([
                'mall_id' => \Yii::$app->mall->id,
                'mch_id' => \Yii::$app->user->identity->mch_id,
                'sign' => $this->plugin_sign,
                'id' => $this->batch_ids,
            ])->column();
            $cats_ids = GoodsCats::find()->select('id')->where([
                'mall_id' => \Yii::$app->mall->id,
                'mch_id' => \Yii::$app->user->identity->mch_id,
                'is_delete' => 0,
            ])->column();
            ...
            GoodsCatRelation::updateAll([
                ...
                ['not in', 'cat_id', $this->cat_ids],
                ['cat_id' => $cats_ids]
            ]);
            $noChange = GoodsCatRelation::find()->select('goods_warehouse_id,cat_id, is_delete')->where([
                'is_delete' => 0,
                'goods_warehouse_id' => $goodsWarehouseIds,
                'cat_id' => $this->cat_ids,
            ])->asArray()->all();
            $add = [];
            for ($i = 0; $i < count($goodsWarehouseIds); $i++) {
                for ($j = 0; $j < count($this->cat_ids); $j++) {
                    $item = [
                        'goods_warehouse_id' => $goodsWarehouseIds[$i],
                        'cat_id' => $this->cat_ids[$j],
                        'is_delete' => 0
                    ];
                    if (in_array($item, $noChange)) {
                        continue;
                    } else {
                        array_push($add, $item);
                    }
            ...
            if (!empty($add)) {
                \Yii::$app->db->createCommand()->batchInsert(
                    ...
                    ['goods_warehouse_id', 'cat_id', 'is_delete'],
                    $add
                )->execute();
            }
            $t->commit();
