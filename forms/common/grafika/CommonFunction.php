...
class CommonFunction
{
    ...
    {
        //$image_url = str_replace("https", "http", $image_url);$image_url = str_replace("localhost", "nginx", $image_url);
            ...
            for ($y = 0; $y < $h; $y++) {
                ...
                if ((($_x * $_x) + ($_y * $_y)) < ($r * $r)) {
                    ...
                    if (!imageistruecolor($src)) {
                        $index = imagecolorsforindex($src, $color);
                        $color = imagecolorallocate($new_pic, $index['red'], $index['green'], $index['blue']);
                    }
     ...
     * @return string
     */
    public static function wechatCode($image_url, $path = './', $margin = 10)
    {
        define('MARGIN', $margin, false);
        ...
        for ($x = 0; $x < $w; $x++) {
            for ($y = 0; $y < $h; $y++) {
                ...
                if (!imageistruecolor($src)) {
                    $index = imagecolorsforindex($src, $color);
                    $color = imagecolorallocate($create_big_pic, $index['red'], $index['green'], $index['blue']);
                }
