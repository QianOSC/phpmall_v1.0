...
class PrintForm extends Model
{
    ...
    public function save()
    {
        ...
        try {
            $this->order_detail_ids = json_decode($this->order_detail_ids,true);
            if(empty($this->order_detail_ids)){
                throw new \Exception('请选择发货商品');
            }
            $result = $this->track();
            return [
    ...
    public function track()
    {
        ...
        if ($cache = $this->getCache($order = $this->getOrder(), $express, $config)) {
            //kd100 格式化
            isset($cache['Order']['LogisticCode']) || $cache['Order']['LogisticCode'] = array_keys($cache['Order']['data']) === range(0, count($cache['Order']['data']) - 1) ? $cache['Order']['data'][0]['kuaidinum'] : $cache['Order']['data']['kuaidinum'];
            isset($cache['Order']['template']) || $cache['Order']['template'] = $cache['Order']['data'][0]['template'][0] ?? '';
            isset($cache['Order']['imgbase64']) || $cache['Order']['imgbase64'] = $cache['Order']['data']['imgBase64'] ?? '';
            return $cache;
        }
    ...
    private function formatDelivery($express_id): array
    {
        ...
        return [
            ...
            'kd100_t_height' => $delivery['kd100_t_height'] ?? '',
            'kd100_t_width' => $delivery['kd100_t_width'] ?? '',
    ...
    private function getCache($order, $express, $config)
    {
        ...
        $expressSingle = OrderExpressSingle::findAll([
            'mall_id' => \Yii::$app->mall->id,
            'ebusiness_id' => $ebusiness_id,
            'order_id' => $order->id,
            'express_code' => $express['code'],
            'order_detail_id' => $this->order_detail_ids
        ]);
        if (empty($expressSingle)
            || count($this->order_detail_ids) !== count($expressSingle)
            || count(array_unique(array_column($expressSingle, 'order'))) > 1) {
            return false;
        }
        $expressSingle = current($expressSingle);
