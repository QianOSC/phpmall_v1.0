...
abstract class BaseForm extends Model
{
    ...
    protected function getGoodsName($goods, $delivery, &$goods_attr)
    {
        ...
        $goodsName = str_replace('%', '%25', $goodsName);
        $goodsName = str_replace('+', '%2B', $goodsName);
        $unit = $goods->goods->goodsWarehouse->unit;
        $goodsName = $goodsName . '（' . trim(join($attr_str), ';') . '）' . '【' . $goods['num'] . ($unit ?: '个') . '】';
