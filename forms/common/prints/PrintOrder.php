...
class PrintOrder extends Model
{
    ...
    public function print($order, $orderId, $printType, int $print_id = 0)
    {
        ...
        $printSetting = $this->getPrintSetting($order, $print_id);
        ...
        foreach ($printSetting as $item) {
            ...
            $orderSendTypeList = \yii\helpers\BaseJson::decode($item->order_send_type);
            $map = ['express','offline', 'city', 'auto'];
            $send_type = $map[$order->send_type];

            if (
                $printType !== 'reprint' && !($orderSendTypeList[$send_type] ?? false)
            ) {
                \Yii::warning('发货方式不支持');
                continue;
            }
            ...
            $data->is_attr = isset($item->is_attr) ? $item->is_attr : 1;
            ...
            try {
                ...
                // TODO 暂时只有一个模板
                $template = $this->getTemplate($data, $item);
    ...
    public function getData($order)
    {
        ...
        $data->erase_price = $this->getErasePrice($order);
        return $data;
    }
    ...
    public function getErasePrice($order)
    {
        $erasePrice = 0;
        ...
        return $erasePrice > 0 ? ('+' . $erasePrice) : $erasePrice;
    }
}
