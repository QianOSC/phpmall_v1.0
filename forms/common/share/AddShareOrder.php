...
{
    public static $isSendTemplate = true;
    public function save($order, $isSendTemplate = true)
    {
        // 判断账户是否有分销权限
        ...
        if (count($shareOrderList) > 0 && $isSendTemplate && self::$isSendTemplate) {
            try {
                if ($firstPrice > 0) {
                    $this->sendTemplate($firstParentUser->user, $firstPrice);
                    $this->sendSmsToUser($firstParentUser->user, $user->nickname, $firstPrice);
                }
                if ($secondPrice > 0) {
                    $this->sendTemplate($secondParentUser->user, $secondPrice);
                    $this->sendSmsToUser($secondParentUser->user, $user->nickname, $secondPrice);
                }
                if ($thirdPrice > 0) {
                    $this->sendTemplate($thirdParentUser->user, $thirdPrice);
                    $this->sendSmsToUser($thirdParentUser->user, $user->nickname, $thirdPrice);
                }
                self::$isSendTemplate = false;
