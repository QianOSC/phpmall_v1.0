...
class CommonShare extends Model
{
     ...
     * @throws \Exception
     */
    public function becomeShareByAuto($order, $scene = 1)
    {
        ...
        if ($shareCondition != 5) {
            ...
            } elseif ($becomeCondition == 4) {
                return $this->becomeShareByTotalConsume($order, $scene);
            } else {
                return true;
            }
        }
    ...
    /**
     * @param Order $order
     * @param int $scene 判断的场景值 1--付款后 2--过售后
     * @return bool
     * @throws \Exception
     * 累计消费满额自动成为分销商
     */
    public function becomeShareByTotalConsume($order, $scene = 1)
    {
        ...
        switch ($scene) {
            case 1:
                $orderCount = $this->getTotalConsume();
                break;
            case 2:
                ...
                $orderCount = round($orderPrice, 2);
                break;
            default:
                return false;
        }
        if ($orderCount < $totalConsume) {
            return false;
        }
        $attributes = [
            'status' => 1,
            'reason' => "累计消费{$totalConsume}元自动成为分销商",
            'apply_at' => mysql_timestamp(),
        ];
        \Yii::error('累计消费' . $totalConsume . '元自动成为分销商');
        return $this->becomeShare($order->user, $attributes);
    }
    ...
    public function getConsume()
    {
        ...
        return price_format($orderPrice);
    }
     ...
     * 单次消费（过售后）
     */
    public function getConsumeBySale()
    {
        $orderList = Order::find()->where([
            ...
            'cancel_status' => 0,
            'status' => 1
        ])->select('order_no')->column();
        $orderPrice = PaymentOrder::find()->where(['is_pay' => 1, 'order_no' => $orderList])
            ->select('max(`amount` - `refund`) as orderPrice')->scalar();
        return price_format($orderPrice);
    }
     ...
     * 无条件自动成为分销商
     */
    public function becomeShareByNone($user)
    {
        $shareCondition = ShareSetting::get($this->mall->id, ShareSetting::SHARE_CONDITION, 1);
        $becomeCondition = ShareSetting::get($this->mall->id, ShareSetting::BECOME_CONDITION, 3);
        if ($shareCondition != 5 || $becomeCondition != 3) {
            return false;
        }
        $attributes = [
            'status' => 1,
        ...
        \Yii::error('无条件自动成为分销商');
        return $this->becomeShare($user, $attributes);
    }
}
