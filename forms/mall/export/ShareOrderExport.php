...
class ShareOrderExport extends BaseExport
{
    ...
    {
        $query = $this->query->with(['user.userInfo', 'shareOrder', 'detail.share', 'clerkUser', 'store'])->orderBy(['created_at' => SORT_DESC]);
    ...
    protected function transform($list)
    {
        ...
        foreach ($list as $key => $item) {
            ...
            $arr['clerk_store'] = $item->store ? $item->store->name : '';
            $arr['clerk_name'] = $item->clerkUser ? $item->clerkUser->user->nickname : '';
