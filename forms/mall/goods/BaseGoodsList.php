    ...
    public function getList()
    {
        ...
        try {
            ...
            if (isset($search['sort_prop']) && $search['sort_prop'] && isset($search['sort_type'])) {
                ...
                } else if ($search['sort_prop'] == 'goodsWarehouse.original_price') {
                    $query->leftJoin(['gw' => GoodsWarehouse::tableName()], 'gw.id=g.goods_warehouse_id');
                    $query->addSelect(['gw.original_price'])->orderBy(['gw.original_price' => $sortType]);
                }
                else {
            ...
            $list = $query->with('goodsWarehouse.cats', 'attr')->select(['g.*'])->page($pagination)->all();
