    ...
    private function sendMpTplMsg($model)
    {
        try {
            if ($model->mch->user) {
                ...
                $data = $form->getPlatformData();
                $adminOpenList = [];
                foreach ($data['admin_open_list'] as $item) {
                    $adminOpenList[] = json_encode($item);
                }

                $templateId = '';
                foreach ($data['template_list'] as $item) {
                    if ($item['key_name'] == 'share_withdraw_tpl') {
                        $templateId = $item['share_withdraw_tpl'];
                    }
                }
                
                $customize = new MpTplMsgCSend();
                $customize->admin_open_list = $adminOpenList;
                $customize->template_id = $templateId;
                $customize->app_id = $data['app_id'];
                $customize->app_secret = $data['app_secret'];
