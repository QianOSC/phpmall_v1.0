...
abstract class BaseOrderForm extends Model
{
    ...
    {
        $exportClass = 'app\\forms\\mall\\export\\OrderExport';
        if (\Yii::$app->user->identity->mch_id > 0) {
            $exportClass = 'app\plugins\mch\forms\mall\\OrderExport';
        }
    ...
    {
        return 'app\\forms\\mall\\order\\OrderForm';
    }

    protected function getFieldsList()
    {
        if (\Yii::$app->user->identity->mch_id > 0) {
            return (new \app\plugins\mch\forms\mall\OrderExport())->fieldsList();
        }
    ...
    protected function where()
    {
        ...
        switch ($this->date_type) {
            ...
            case 'finish_time':
                if ($this->date_start) {
                    $query->andWhere(['>=', 'o.updated_at', $this->date_start]);
                }

                if ($this->date_end) {
                    $query->andWhere(['<=', 'o.updated_at', $this->date_end]);
                }
                break;
