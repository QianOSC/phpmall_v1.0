    ...
    public function search()
    {
        ...
        foreach ($list as $item) {
            ...
            if ($item->is_virtual == 1) {
                ...
                $newItem['platform_text'] = '后台';
                $newItem['platform_icon'] = 'statics/img/mall/site.png';
            } else {
                ...
                $newItem['platform_text'] = $userPlatformDetail['text'];
                $newItem['platform_icon'] = $userPlatformDetail['platform_icon'];
            }
            try {
                $newItem['pic_url'] = \yii\helpers\BaseJson::decode($item->pic_url);
            } catch (\Exception $exception) {
                $newItem['pic_url'] = [];
            }
            try {
                $goodsInfo = \yii\helpers\BaseJson::decode($item->detail->goods_info);
