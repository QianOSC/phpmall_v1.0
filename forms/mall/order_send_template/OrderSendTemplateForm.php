...
class OrderSendTemplateForm extends Model
{
    ...
    const EDIT_DEFAULT = 'editDefault';
    ...
    public function editDefault()
    {
        ...
        try {
            ...
            $template = OrderSendTemplate::find()->where($where)->andWhere([
                'id' => $this->id,
            ])->one();
            if (empty($this->id) || empty($template)) {
                throw new \Exception('模板不存在');
            }
            if ($this->is_default == 1) {
                OrderSendTemplate::updateAll(['is_default' => 0], $where);
            }
            $template->is_default = $this->is_default;
            if (!$template->save()) {
