...
class RoleUserForm extends Model
{
    ...
    public function getList()
    {
        ...
        $operatorIds = UserIdentity::find()->alias('i')->where([
            'OR',
            ['i.is_operator' => 1],
            ['i.is_supplier' => 1]
        ])->select('i.user_id');
        ...
        $query = User::find()->alias('u')
            ->leftJoin(['a' => AuthRoleUser::tableName()], 'a.user_id = u.`id`')
            ->leftJoin(['r' => AuthRole::tableName()], 'r.id = a.role_id')
            ->select('u.*, r.name as supplier_name')
            ->where(['u.is_delete' => 0, 'u.mall_id' => \Yii::$app->mall->id])
            ->andWhere(['u.id' => $operatorIds]);

        if ($this->keyword) {
            $query->andWhere(['like', 'u.nickname', $this->keyword]);
        }

        $list = $query->page($pagination, 10)->orderBy('u.created_at DESC')->asArray()->all();
