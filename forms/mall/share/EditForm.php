...
class EditForm extends Model
{
    ...
    public function getUser()
    {
        ...
        $list = User::find()->alias('u')->with(['userInfo', 'userPlatform'])->where([
            ...
            ->keyword($this->keyword !== '', ['like', 'BINARY(u.nickname)', $this->keyword])
            ->addOrderBy(['u.nickname' => $this->keyword])
            ->apiPage(20)->select('u.id,u.nickname')->all();
        array_walk($list, function (&$item) {
            $platform = $item->userInfo ? $item->userInfo->platform : '';
            $item->nickname = PlatformConfig::getInstance()->getPlatformText($item) . '（' . $item->nickname . '）';
        });
        return [
