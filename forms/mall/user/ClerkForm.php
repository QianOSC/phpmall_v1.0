...
class ClerkForm extends Model
{
    ...
    public function search()
    {
        ...
        if ($this->flag == "EXPORT") {
            $query = clone $form->query;

            $queueId = CommonExport::handle([
                'export_class' => 'app\\forms\\mall\\export\\ClerkExport',
                'params' => [
                    'query' => $query,
        ...
        $userList = User::find()->with(['userInfo', 'userPlatform'])
            ->where(['mall_id' => \Yii::$app->mall->id, 'id' => array_column($list, 'user_id')])
            ->all();
        $platformList = [];
        foreach ($userList as $user) {
            $platformList[$user->id] = PlatformConfig::getInstance()->getPlatformIcon($user);
        }
        ...
        foreach ($list as &$item) {
            ...
            $item['platform_icon'] = $platformList[$item['user_id']];
        }
