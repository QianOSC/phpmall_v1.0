...
class UserForm extends Model
{
    ...
    public function shareUser()
    {
        ...
        try {
            ...
            foreach ($list as $k => $v) {
                $list[$k]['new_name'] = "(".$v['id'].")" . $v['nickname'];
                if ($v['name']) {
    ...
    public function searchUser()
    {
        ...
        foreach ($list as $item) {
            ...
            $newItem['nickname'] = $platformConfig->getPlatformText($item) . '（' . $item->nickname . '）';
            $newList[] = $newItem;
        }
    ...
    public function getList()
    {
        ...
        if ($this->flag == "EXPORT") {
            $queueId = CommonExport::handle([
                'export_class' => 'app\\forms\\mall\\export\\UserExport',
                'params' => [
                    'query' => $query,
        ...
        $userCenterConfig = UserCenterConfig::getInstance()->getConfig();
        $memberList = array_column($mall_members, null, 'level');
        $platformConfig = new PlatformConfig();
        if (!empty($list)) {
            $userList = User::find()->with(['userInfo', 'userPlatform'])
                ->where(['id' => array_column($list, 'user_id')])->all();
        ...
        foreach ($list as &$v) {
            ...
            $userPlatform = $platformConfig->getPlatformDetail($userList[$v['user_id']]);
            if ($this->is_change_name) {
                $v['nickname'] = $userPlatform['text'] . '（' . $v['nickname'] . '）';
            }
            $v['openid'] = $userPlatform['openid'];
            $v['icon'] = $userPlatform['icon'];
            $v['platform_icon'] = $userPlatform['platform_icon'];
    ...
    public function getDetail()
    {
        ...
        $newList = [
            ...
            'new_name' => $user->parent ? "(".$user->parent->id.") " . $user->parent->nickname : '总店',
    ...
    public function balanceLog()
    {
        ...
        if ($this->flag == "EXPORT") {
            $queueId = CommonExport::handle([
                'export_class' => 'app\\forms\\mall\\export\\BalanceLogExport',
                'params' => [
                    'query' => $query,
                    'fieldsKeyList' => $this->fields,
                ]
    ...
    public function integralLog()
    {
        ...
        if ($this->flag == "EXPORT") {
            $queueId = CommonExport::handle([
                'export_class' => 'app\\forms\\mall\\export\\IntegralExport',
                'params' => [
                    'query' => $query,
                    'fieldsKeyList' => $this->fields,
                ]
