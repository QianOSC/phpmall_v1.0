...
class We7Branch extends BaseBranch
{
    ...
    public function childPermission($adminInfo)
    {
        if ($adminInfo->is_default == 1) {
            ...
            if ($status && $status == 1) {
                ...
                if ($data) {
                    $permission = array_merge($data['mall_permissions'], $data['plugin_permissions']);
                }
            } else {
                $permission = [];
            }
    ...
    public function getSecondaryPermission($adminInfo)
    {
        if ($adminInfo->is_default == 1) {
            ...
            if ($status && $status == 1) {
                ...
                if ($data) {
                    $permission = array_merge($data['secondary_permissions'], $data['secondary_permissions']);
                }
            } else {
    ...
    {
        return parent::checkMallUser($user) && !$user->adminInfo;
    }
}
