...
use app\jobs\OrderAutoComments;
use app\models\Mall;
...
abstract class BaseOrderSalesHandler extends BaseOrderHandler
{
    ...
    protected function level()
    {
        try {
            ...
            $this->memberLevel($orderMoneyCount);
            $this->shareLevel($orderMoneyCount);
            return true;
        } catch (\Exception $e) {
    ...
    protected function memberLevel($orderMoneyCount)
    {
        try {
            ...
            \Yii::warning('消费升级会员等级错误' . $exception->getMessage());
            return false;
        }
    }

    protected function shareLevel($orderMoneyCount)
    {
        try {
            \Yii::warning('分销等级升级，当前用户消费总额' . $orderMoneyCount);
            $commonShareLevel = CommonShareLevel::getInstance();
            $commonShareLevel->userId = $this->order->user_id;
            $commonShareLevel->levelShare(CommonShareLevel::TOTAL_CONSUME);
            return true;
        } catch (\Exception $exception) {
            \Yii::warning('消费升级分销等级错误' . $exception->getMessage());
            return false;
        }
    }
    ...
    protected function becomeShare()
    {
        try {
            ...
            $commonShare->becomeShareByAuto($this->event->order, 2);
            return true;
        } catch (\Exception $exception) {
            \Yii::error('过售后成为分销商: ' . $exception->getMessage());
            return false;
        }
    }
