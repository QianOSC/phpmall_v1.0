...
class Mall extends ModelActiveRecord
{
    ...
    public function getMallSetting($columns = [])
    {
        if ($this->mallAllOptions) {
            ...
            // 需要被转成数字类型的
            $arr = [
                'add_app_bg_transparency', 'add_app_bg_radius', 'is_show_cart', 'is_show_goods_name', 'is_show_stock',
                'is_sales', 'is_must_login', 'is_goods_video', 'is_open', 'open_type', 'is_auto_open',
                'show_contact_type', 'is_video_number', 'is_video_number_member', 'is_remind_sell_time'
            ];
            // 需要被json_decode的
            $jsonArr = [
                'good_negotiable', 'payment_type', 'send_type', 'business_time_type_week', 'business_time_type_day',
                'quick_customize_new_params', 'send_type_desc', 'week_list', 'time_list', 'video_number_member_list',
                'video_number_template_list', 'customer_service_list', 'custom_theme_color'
            ];
            foreach ($detail['option'] as $k => $item) {
                if (in_array($item['key'], $arr)) {
                    $item['value'] = (int)$item['value'];
                }
                if (in_array($item['key'], $jsonArr)) {
                    ...
                    } elseif ($item['key'] == 'quick_customize_params') {
                        $newOption[$item['key']] = '';
                    } else {
    ...
    public function getDefault()
    {
        ...
        return [
            ...
            'is_list_underline_price' => '1', //列表划线价
            'is_express' => '1', //快递
            ...
            'kd100_secret' => '',//快递100
            'kd100_siid' => '', //快递100
            'kd100_yum' => '1', //快递100云打印
            'print_type' => '', //那个电子面单

            'is_open' => 1, //是否开业 1、营业 2、打烊
            'open_type' => 1, //营业时间段 1、全天 2、自定义
            'week_list' => [],// 1.星期一 2.星期二 ...
            'time_list' => [], // [['07:00', '12:00'],['13:00', '17:00']]
            'is_auto_open' => 1, // 设置是否自动开业 1、不设置 2、自定义
            'auto_open_time' => '', // 设置自动开业时间
            'show_contact_type' => 0, // 显示客服类型 0--不显示 1--显示在线客服 2--显示客服外链

            'is_video_number' => 0,
            'video_number_app_id' => '',
            'video_number_app_secret' => '',
            'video_number_share_title' => '',
            'is_video_number_member' => 0,
            'video_number_member_list' => [],
            'video_number_template_list' => [],
            'video_number_user_1' => '',
            'video_number_user_2' => '',

            'kdniao_select_type' => 'free',
            'customer_service_list' => [],// 客服微信列表 最多10个
            'show_goods_auth' => '-1',
            'buy_goods_auth' => '-1',
            //默认好评功能
            'has_order_evaluate' => 0,
            'order_evaluate_day' => 0,
