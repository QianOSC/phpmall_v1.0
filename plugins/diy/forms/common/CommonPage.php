    ...
    public function getPage($pageId = null, $isIndex = false)
    {
        ...
        if (!empty($page['navs'])) {
            try {
                ...
                foreach ($page['navs'] as &$nav) {
                    if (!empty($nav['template']['data'])) {
                        ...
                        foreach ($nav['template']['data'] as $dIndex => &$item) {
                            ...
                            //商品批发
                            if ($item['id'] == 'wholesale') {
                                $diyWholesaleForm = new DiyFlashSaleForm();
                                $wholesaleGoodsIds = array_merge(
                                    $diyWholesaleForm->getGoodsIds($item['data']),
                                    $wholesaleGoodsIds
                                );
                            }
                        }
                ...
                //商品批发
                if (isset($diyWholesaleForm)) {
                    $wholesale = $diyWholesaleForm->getGoodsById($wholesaleGoodsIds);
                }
                // 直播
                ...
                foreach ($page['navs'] as $index => &$nav) {
                    if (!empty($nav['template']['data'])) {
                        foreach ($nav['template']['data'] as &$item) {
                            ...
                            //限时购买
                            if ($item['id'] == 'wholesale') {
                                $item['data'] = $diyWholesaleForm->getNewGoods($item['data'], $wholesale);
                            }
