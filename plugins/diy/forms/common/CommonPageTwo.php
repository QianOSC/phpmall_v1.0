...
class CommonPageTwo extends Model
{
                ...
                if ($item['id'] === 'module') {
                    if (isset($item['data']['list']) && is_array($item['data']['list'])) {
                        foreach ($item['data']['list'] as $key1 => $module) {
                            $module['data'] = array_filter($module['data'], function ($i) {
                                return $i['id'] !== 'background';
                            });
    ...
    //批发
    protected function sWholesale(bool $is_show, array $arr)
    {
        if ($is_show) {
            $diyWholesaleForm = new DiyWholesaleForm();
            $arr['data'] = $diyWholesaleForm->getNewGoods($arr['data'], $this->wholesale);
            return $arr;
        } else {
            $diyWholesaleForm = new DiyWholesaleForm();
            $wholesaleGoodsIds = [];
            foreach ($arr as $goods) {
                $wholesaleGoodsIds = array_merge(
                    $diyWholesaleForm->getGoodsIds($goods['data']),
                    $wholesaleGoodsIds
                );
            }
            $this->wholesale = $diyWholesaleForm->getGoodsById($wholesaleGoodsIds);
        }
    }
