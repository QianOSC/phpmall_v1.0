...
//                    break;
    ...
    public function install()
    {
        ...
        try {
            ...
            $data = $detail;
//            $detail = \Yii::$app->cloud->template->getDetail([
//                'id' => $this->template_id
//            ]);
//            if (!$detail['order']) {
//                throw new \Exception('模板没有下单，请先下单');
//            }
//            if ($detail['order']['is_pay'] != 1) {
//                throw new \Exception('模板订单未付款，请先联系管理员付款');
//            }
...
//            $data = \Yii::$app->cloud->template->install($this->template_id);
            $issue = new Issue();
            $issue->type = 'decode';
            ...
            /** @var CoreTemplateType[] $templateType */
//            $templateType = CoreTemplateType::find()->where([
//                'template_id' => $detail['id']
//            ])->all();
//            $local = array_column($templateType, 'type');
//            $same = array_intersect($local, $detail['range']); // 获取本地存储的和远端的交集
//            $insert = array_diff($detail['range'], $same);
//            $delete = array_diff($local, $same);
//            foreach ($insert as $range) {
//                $model = new CoreTemplateType();
//                $model->template_id = $detail['id'];
//                $model->type = $range;
//                $model->is_delete = 0;
//                $model->save();
//            }
//            foreach ($templateType as $item) {
//                if (in_array($item->type, $delete)) {
//                    $item->delete();
//                }
//            }
            return [
