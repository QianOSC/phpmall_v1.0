...
abstract class MchEditFormBase extends Model
{
    ...
    public function rules()
    {
        return [
            ...
            [['user_id', 'mch_common_cat_id', 'transfer_rate', 'sort', 'id', 'status', 'is_recommend',
                'province_id', 'city_id', 'district_id', 'is_open', 'open_type', 'is_auto_open'], 'integer'],
            [['mobile', 'logo', 'service_mobile', 'password', 'auto_open_time'], 'string', 'max' => 255],
    ...
    protected function setStore()
    {
        ...
        if (is_array($this->week_list)) {
            $this->week_list = json_encode($this->week_list);
        }

        $weekList = is_string($this->week_list) ? json_decode($this->week_list, true) : [];
        if ($this->is_open == 1 && $this->open_type == 2 && empty($weekList)) {
            throw new \Exception('请选择营业日期星期');
        }

        if (is_array($this->time_list)) {
            $this->time_list = json_encode($this->time_list);
        }
        ...
        $store->extra_attributes = json_encode([
            ...
            'week_list' => $weekList,
            'time_list' => $this->time_list ? json_decode($this->time_list, true) : [],
