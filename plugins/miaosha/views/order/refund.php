    ...
    <el-card shadow="never" style="border:0" body-style="background-color: #f3f3f3;padding: 10px 0 0;" v-loading="loading">
        ...
        <div slot="header">
            ...
            <div style="float: right;margin: -5px 0">
                <app-export-dialog action_url='index.php?r=plugin/miaosha/mall/order/refund'
...
<script>
    new Vue({
        ...
        methods: {
            ...
            getList() {
                this.loading = true;
                loadList('plugin/miaosha/mall/order/refund').then(e => {
                    this.$message({
                        message: '请求成功',
                        type: 'success'
                    });
            ...
            pageChange(page) {
                this.loading = true;
                this.list = [];
                loadList('plugin/miaosha/mall/order/refund',page).then(e => {
                    this.loading = false;
                    this.list = e.list;
                    let detail = [];
                    for(let i = 0;i < this.list.length;i++) {
            ...
            handleClick(e) {
                ...
                request({
                    params: {
                        r: 'plugin/miaosha/mall/order/refund',
                        status: status,
                    },
                }).then(e => {
                    this.loading = false;
