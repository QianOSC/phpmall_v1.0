...
<div id="app" v-cloak>
    <el-card shadow="never" style="border:0" body-style="background-color: #f3f3f3;padding: 10px 0 0;"
        ...
        <div class="table-body">
            <div style="display: flex">
                ...
                <el-alert type="info" style="height: 32px;padding: 0 16px;margin-left: 10px;width: auto"
                          :closable="false">
                    <template>
                        温馨提示：商品分类修改至“
                        <el-button type="text" @click="$navigate({r:'mall/cat/index'}, true)">商品管理-分类</el-button>
                        ”修改
                    </template>
                </el-alert>
            </div>
            <el-table :data="form" border style="width: 100%">
                ...
                <el-table-column label="分类名称">
                    <template slot-scope="scope">
                        {{scope.row.cats ? scope.row.cats.name : ''}}
                    </template>
                </el-table-column>
                <el-table-column label="图标">
                    <template slot-scope="scope">
                        <app-image mode="aspectFill" :src="scope.row.cats ? scope.row.cats.pic_url : ''"></app-image>
                    </template>
                </el-table-column>
    ...
    <!-- 更新分类 -->
    <el-dialog title="添加分类" :visible.sync="dialogSortAdd" width="30%">
        <el-form :model="sortAddForm" label-width="80px" :rules="sortAddFormRules" ref="sortAddForm">
            <el-form-item label="分类" prop="cat_list" size="small">
                <el-cascader v-model="sortAddForm.cat_list" :options="cats" :show-all-levels="false" change-on-select
                             :props="defaultProps"></el-cascader>
            </el-form-item>
            <el-form-item label="排序" prop="sort" size="small">
                <el-input v-model="sortAddForm.sort"></el-input>
            </el-form-item>
        </el-form>
        <div slot="footer" class="dialog-footer">
            <el-button @click="dialogSortAdd = false">取消</el-button>
            <el-button :loading="btnLoading" type="primary" @click="sortAddSubmit">提交</el-button>
        </div>
    </el-dialog>
</div>
<script>
    const app = new Vue({
        ...
        methods: {
            ...
            sortAddSubmit() {
                this.$refs.sortAddForm.validate((valid) => {
                    if (valid) {
                        ...
                        request({
                            params: {
                                r: 'plugin/pintuan/mall/cats/edit',
                            },
                            method: 'post',
                            data: para,
                        }).then(e => {
            ...
            sortSubmit(row) {
                ...
                request({
                    params: {
                        r: 'plugin/pintuan/mall/cats/edit-sort',
                    },
                    method: 'post',
                    data: para,
                }).then(e => {
            ...
            destroy: function (column) {
                this.$confirm('确认删除该记录吗?', '提示', {
                    ...
                    request({
                        params: {
                            r: 'plugin/pintuan/mall/cats/destroy',
                        },
                        data: {id: column.id},
                        method: 'post'
                    }).then(e => {
            ...
            getList() {
                ...
                request({
                    params: {
                        r: 'plugin/pintuan/mall/cats/index',
                        page: this.page,
