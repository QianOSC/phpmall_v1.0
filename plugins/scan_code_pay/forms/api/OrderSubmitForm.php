...
class OrderSubmitForm extends Model
{
    ...
    public function rules()
    {
        return [
            ...
            [['use_integral', 'coupon_id', 'clerk_user_id'], 'integer'],
            [['remark'], 'string']
        ];
    }
    ...
    public function submit()
    {
        ...
        try {
            ...
            if ($this->clerk_user_id) {
                $clerkUser = ClerkUser::find()->where([
                    'user_id' => $this->clerk_user_id,
                    'mall_id' => \Yii::$app->mall->id,
                    'is_delete' => 0
                ])->with('storeRelation')->one();


                if ($clerkUser && $clerkUser->storeRelation) {
                    $order->clerk_id = $clerkUser->id;
                    $order->store_id = $clerkUser->storeRelation->store_id;
                }
            }
