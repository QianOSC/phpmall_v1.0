...
use app\plugins\stock\forms\export\CashExport;
...
class CashListForm extends Model
{
    ...
    public function search()
    {
        ...
        $list = $query->orderBy(['s.created_at' => SORT_DESC])
        ...
        foreach ($list as $item) {
            ...
            $newItem = [
                ...
                'user' => [
                    'name' => $item->stockUser->name,
                    'mobile' => $item->stockUser->phone,
    ...
    {
        $query = StockCash::find()
            ->alias('s')
            ->where(['s.mall_id' => $this->mall->id, 's.is_delete' => 0])
            ->joinWith(['user u' => function ($query) {
                ...
                'stockUser c' => function ($query) {
                    ...
                    if ($this->keyword && $this->search_type == 3) {
                        $query->andWhere([
                            'c.phone' => $this->keyword
                        ]);
                    }
                }])
            ->keyword($this->status >= 0, ['s.status' => $this->status])
            ->keyword($this->user_id, ['s.user_id' => $this->user_id]);

        if ($this->date_start) {
            $query->andWhere(['>=', 's.created_at', $this->date_start]);
        }

        if ($this->date_end) {
            $query->andWhere(['<=', 's.created_at', $this->date_end]);
        }
