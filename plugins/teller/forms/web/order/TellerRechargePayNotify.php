...
class TellerRechargePayNotify extends RechargePayNotify
{
    public function notify($paymentOrder)
    {
        try {
            ...
            $desc = '充值余额：' . $reOrder->pay_price . '元,赠送：' . $reOrder->send_price . '元';
            $desc = $this->sendMember($user, $reOrder, $desc);
            $this->sendBalance($user, $reOrder, $desc);
            $this->sendIntegral($user, $reOrder);
