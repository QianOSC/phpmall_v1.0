...
class CommonVip extends Model
{
    ...
    {
        if (self::$isLookUp && empty(self::$userInfo)) {
            return [
                'is_vip_card_user' =>  0
            ];
        }
        ...
        return [
            ...
            'vip_card_user' => isset($user->id) ? $user : []
        ];
    }
    ...
    public function generateOrders($id, $token, $orderDetailVipCardInfoData = [], $isOtherPluginSend = false, $sign = '', $payType = 1, $order_form = [])
    {
        ...
        $order->token = $token;
        $order->status = 1;
        $order->platform = \Yii::$app->appPlatform ?? '';
        if (!$order->save()) {
