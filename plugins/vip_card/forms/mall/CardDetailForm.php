...
class CardDetailForm extends Model
{
    ...
    public function getDetail()
    {
        ...
        $vipCardsNum = array_column($detail['vipCards'], 'total_count', 'id');
        foreach ($detail['cards'] as $k => &$item) {
            if (isset($vipCards[$item['card_id']]) && isset($vipCardsNum[$item['card_id']]) && $vipCardsNum[$item['card_id']] != 0) {
        ...
        $vipCouponsNum = array_column($detail['vipCoupons'], 'total_count', 'id');
        foreach ($detail['coupons'] as $k => &$item) {
            if (isset($vipCoupons[$item['coupon_id']]) && isset($vipCouponsNum[$item['coupon_id']]) && $vipCouponsNum[$item['coupon_id']] != 0) {
