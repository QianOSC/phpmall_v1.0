        ...
        <el-card v-loading="listLoading" class="box-card" shadow="never" style="border:0"
            ...
            <div class="table-body">
                ...
                <el-table
                    ...
                    <el-table-column prop="id" label="ID" width="80" :sortable="sign !== 'exchange' ? 'custom':false"></el-table-column>
                    <el-table-column v-if="!is_mch && sign !== 'exchange'" prop="sort" :width="sort_goods_id != id ? 100 : 80" label="排序"
                                     sortable="false">
                                        ...
                                        <div style="position: absolute;width: 250px;height: 50px;overflow: hidden" flex="dir:left">
                                            ...
                                            <el-tooltip class="item"
                                                ...
                                                <template slot="content">
                                                    <div style="width: 320px;" v-if="scope.row.goodsWarehouse.name.length < 30" flex="cross:center">{{scope.row.goodsWarehouse.name}}</div>
                                            ...
                                            -->
                                            <template slot="content" style="line-height:50px;">
                                                <div style="width: 320px;" v-if="scope.row.goodsWarehouse.name.length < 30" flex="cross:center">{{scope.row.goodsWarehouse.name}}</div>
                                                <div style="width: 320px;" v-else flex="cross:center">{{scope.row.goodsWarehouse.name}}</div>
                                            </template>
                                            <!--
                    ...
                    <el-table-column v-if="goods_url == 'plugin/integral_mall/mall/goods/index'" prop="goodsWarehouse.original_price" label="原价" sortable="custom">
                        <template slot-scope="scope">
                            <div>{{scope.row.goodsWarehouse.original_price}}</div>
                        </template>
                    </el-table-column>
                    <el-table-column v-else prop="price" :label="sign !== 'exchange'?'售价':'价格'" :sortable="sign !== 'exchange' ? 'custom':false">
                        <template slot-scope="scope">
                    ...
                    <el-table-column prop="goods_stock" label="库存" :sortable="sign !== 'exchange' ? 'custom':false">
                        <template slot-scope="scope">
                        ...
                        <template slot="header">
                            <div style="vertical-align: middle;">已出售量
                                <el-tooltip class="item" effect="dark"
                                            content="已出售量=实际销量+商品设置的已出售量，按实际销量排序"
                                            placement="top">
                    ...
                    <el-table-column v-if="!is_mch" prop="created_at" width="160" label="添加时间"></el-table-column>
