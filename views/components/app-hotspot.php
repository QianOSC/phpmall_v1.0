...
<style>
    ...
    .app-hotspot .input-with-select {
        width: 80%;
    }

    .app-hotspot .el-radio {
                    ...
                    <div class="right" v-if="index > -1">
                        ...
                        <el-form-item label="热区属性" v-if="mode == 'auth'">
                            <el-radio-group v-model="hotspotList[index].open_type">
                                ...
                                <el-radio label="link" :disabled="radioDisabled('link')">自定义链接</el-radio>
                            </el-radio-group>
                        </el-form-item>

                        <el-form-item prop="params" label="自定义按钮" v-if="mode == 'auth'">
                            <el-input size="small" v-model="hotspotList[index]['link']['url']"
                                      class="input-with-select"
                                      autocomplete="off" disabled>
                                <app-pick-link slot="append" @selected="selectPageUrl">
                                    <el-button size="mini" @click="searchIndex(index)">选择链接</el-button>
                                </app-pick-link>
                            </el-input>
                        </el-form-item>
                    </div>
                    <div v-if="index == -1 && hotspotList.length == 0" style="padding: 20px 40px;">请先在左侧蓝框内用鼠标划出热区范围
                    </div>
...
<script>
    Vue.component('app-hotspot', {
        ...
        methods: {
            ...
            selectPageUrl(e) {
                for (let item of e) {
                    this.hotspotList[this.tempIndex].link = {
