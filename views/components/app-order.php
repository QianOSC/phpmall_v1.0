...
<template id="app-order-list" ref="appOrder">
    <div class="app-order-list" style="margin-bottom: 20px;">
        <div class="header-box" v-if="isShowHeader">
            <div class="title">
                <slot name="orderTitle">{{titleLabel}} - {{tableTitle}}</slot>
            </div>
                ...
                <el-card
                    ...
                    <div slot="header" class="app-order-head" flex="cross:center">
                        ...
                        <div class="app-order-user" flex="cross:center">
                            <img width="24px" height="24px" :src="item.platform_icon" alt="">
                        ...
                        <div class="app-order-offline" flex="dir:left wrap:wrap">
                            ...
                            <template>
                                <el-tag size="small" type="primary" effect="dark" v-if="item.delivery_type == 0">
                                    {{item.delivery_time}}
                                </el-tag>
                                <el-tag size="small" type="danger" effect="dark" v-if="item.delivery_type == 1">
                                    {{item.delivery_time}}
                                </el-tag>
                            </template>
                    ...
                    <div class="app-order-body">
                                ...
                                <div>
                                    ...
                                    <el-popover
                                        ...
                                        <el-form class="app-order-price-item" label-width="100px" :model="item">
                                            ...
                                            <el-form-item label="满减抵扣" v-if="item.full_reduce_price != 0.00">
                                                <span class="important">-{{ item.full_reduce_price }}元</span>
                                            </el-form-item>
                                            <el-form-item label="订单改价" v-if="item.back_price != 0.00">
                                            ...
                                            <template v-if="item.plugin_data && item.plugin_data.discount_list">
                                                <el-form-item v-for="pluginData in item.plugin_data.discount_list"
                                                    ...
                                                    <span class="important" v-if="pluginData.value > 0">-{{ pluginData.value }}元</span>
                                                    <span class="addPrice" v-else>+{{ -pluginData.value }}元</span>
                                                </el-form-item>
                                            </template>
                                        </el-form>
                        ...
                        <div v-if="isShowAction" class="app-order-info" :style="{width: showMoreInfo ? orderTitle[3].width : orderTitle[2].width}"
                            ...
                            <div flex="wrap:wrap cross:center">
                                ...
                                <!--多个物流信息订单 需到订单详情修改 -->
                                <template
                                        v-else-if="item.detailExpress && item.detailExpress.length >= 1 && item.cancel_status != 1 && item.is_confirm == 0 && item.is_recycle == 0 && isShowSend && item.status != 0 && item.is_update_send == 1">
                                    <el-tooltip class="item" effect="dark" :content="item.send_type == 2 ? '修改配送员' : '修改快递单号'" placement="top">
                                        <img class="app-order-icon"
                    ...
                    <div class="card-footer" v-if="isShowCardfooter">
                        <template v-if="item.goods_type == 'ecard'">
                            <div style="margin: 10px 0;">收货人: {{item.name}} 电话：{{item.mobile}}</div>
                        </template>
                        <template v-if="item.sign == 'scan_code_pay'">
                            <div style="margin: 10px 0;">
                                <template v-if="item.clerk">
                                    <span>核销员:</span>
                                    <span>{{item.clerk.nickname}}</span>
                                </template>
                                <template v-if="item.store">
                                    <span style="margin-left: 10px;">核销门店:</span>
                        ...
                        <template v-if="item.send_type == 1">
                            ...
                            <div style="margin: 10px 0;">收货人: {{item.name}} 电话：{{item.mobile}}</div>
                            <div v-if="item.orderClerk && item.orderClerk.clerk_remark" style="margin: 10px 0;">核销备注: {{item.orderClerk.clerk_remark}}</div>
                        </template>
        ...
        <el-dialog
            ...
            <div v-for="(expressSingle, index) in newExpressSingle"
                ...
                <div flex="dir:top">
                    ...
                    <div flex="dir:left" style="margin-top: 10px;flex-wrap: wrap">
                        ...
                        <img v-for="(goods, index) in expressSingle.goods_list"
                             ...
                             class="goods-pic"
                             :src="goods.cover_pic">
                        <span style="margin-top: auto">等共{{ expressSingle.goods_list.length}}个商品</span>
                    </div>
                </div>
            </div>
        </el-dialog>
                    ...
                    <div>
                        ...
                        <div v-for="good in item.detail"
                            ...
                            <div :style="tableWidth" style="display: flex;" v-if="printPar.goodsInf.name || printPar.goodsInf.attr || printPar.goodsInf.unitPrice">
                               <div
                                       :style="{width: printPar.goodsInf.name && printPar.goodsInf.attr && printPar.goodsInf.unitPrice ? '50%' : printPar.goodsInf.name && (!printPar.goodsInf.attr || !printPar.goodsInf.unitPrice) ? '75%' :'100%'}"
                                       style="min-width: 0;flex-grow: 1; word-break: break-all;flex-shrink: 1;-webkit-box-flex: 1;word-wrap:break-word;box-sizing:border-box;word-wrap: break-word;border-left: 1px solid #000000;padding:  10px 10px 10px 0.5%;font-size:10px;position: relative"  v-if="printPar.goodsInf.name">
                                   {{good.name}}
                               </div>
                               <div
                                       :style="
                                     {
                                       ...
                                       style="min-width: 0;flex-grow: 1; word-break: break-all;flex-shrink: 1;-webkit-box-flex: 1;word-wrap:break-word;box-sizing:border-box;word-wrap: break-word;border-left: 1px solid #000000;font-size:10px;padding: 10px 10px 10px 0.5%;position: relative" v-if="printPar.goodsInf.attr">
                                   {{good.attr}}
                               </div>
                    ...
                    if (e.data.code === 0) {
                        ...
                        newList.forEach(function (item) {
                            ...
                            item.detail.forEach(function (d){
                               if(d.form_data)
                               {
                                   d.form_obj = JSON.parse(d.form_data);
                                   // console.log(d.form_obj.length);
                                   // d.form_obj.forEach(function (f){
                        ...
                        var supplier = e.data.data.supplier;
                        if(supplier != undefined && supplier != '')
                        {
                            this.tableTitle = supplier;
                        }else{
                            this.tableTitle = '';
                        }
                        ...
                        let send_type_list = e.data.data.send_type_list;
                        if (send_type_list && send_type_list.length > 0) {
