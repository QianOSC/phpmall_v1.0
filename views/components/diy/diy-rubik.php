            ...
            data: {
                ...
                handler(newVal, oldVal) {
                    if (this.data.style === 0) {
                        let img = new Image()
                        img.src = this.data.list[0].pic_url
                        const vm = this
                        img.onload = () => {
                            console.log(img.width) // 打印图片信息
                            console.log(img.height) // 打印图片信息
                            this.data.a_height = (img.height * 750 / img.width).toFixed(2);
                        }
                    }
                    this.$emit('input', this.data, oldVal)
                },
            },
            'data.w': {
                handler(newVal, oldVal) {
