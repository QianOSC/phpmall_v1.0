...
<div id="app" v-cloak>
    <el-card shadow="never" style="border:0" body-style="background-color: #f3f3f3;padding: 10px 0 0;"
                            ...
                            <el-tab-pane :label="item.name" :name="item.key">
                                <div class="module-box" flex="wrap:wrap">
                                    <div v-for="item in item.list" class="module-item-box" flex="dir:top">
                                        ...
                                        <div flex="dir:left box:mean" class="edit-box">
                                            ...
                                            text-align: center;line-height: 33px;">
                                                    <el-tooltip :disabled="dialogFormVisible" effect="dark" content="添加" placement="top">
                                                        <img src="statics/img/mall/plus.png" alt="">
                                                    </el-tooltip>
                                                </div>
                                            </div>
                                            <div v-if="item.is_edit == 1 && item.key != 'video'" @click="openDialogForm(item)" flex="main:center cross:center">
        ...
        <el-dialog class="edit" title="编辑" :visible.sync="dialogFormVisible"
                   :class="dialogForm.key === 'notice' ? 'dialog-width' : ''" :before-close="formClose">
            <el-form @submit.native.prevent :model="dialogForm" :rules="dialogRule" ref="dialogForm" label-width="150px" size="small">
                <template v-if="dialogForm.key === 'notice'">
                    <el-form-item label-width="120px" label="公告名称" prop="notice_name">
                        <el-input v-model="dialogForm.notice_name"></el-input>
                    </el-form-item>
                    <el-form-item label-width="120px" label="公告内容" prop="notice_content">
                        <el-input type="textarea" :row="3" v-model="dialogForm.notice_content"></el-input>
                    </el-form-item>
                    <el-form-item label-width="120px" label="公告背景色" prop="notice_bg_color">
                        <el-color-picker
                    ...
                    <el-form-item label-width="120px" label="公告文字颜色" prop="notice_text_color">
                        <el-color-picker
                    ...
                    <el-form-item label-width="120px" label="图标" prop="notice_url">
                ...
                <template v-if="dialogForm.key === 'topic'">
                    <el-form-item label-width="120px" label="专题显示数量" prop="topic_num">
                    ...
                    <el-form-item label-width="120px" v-if="dialogForm.topic_num == 1" label="图标(1个专题)" prop="topic_url">
                    ...
                    <el-form-item label-width="120px" v-if="dialogForm.topic_num == 2" label="图标(2个专题)" prop="topic_url_2">
                    ...
                    <el-form-item label-width="120px" label="专题标签" prop="label_url">
                ...
                <template v-if="dialogForm.key === 'video'">
                    <el-form-item label-width="100px" label="视频文件" prop="video_url">
                    ...
                    <el-form-item label-width="100px" label="封面图" prop="video_pic_url">
                ...
                <template v-if="activeName === 'plugin'">
                    <el-form-item label-width="100px" label="样式设置">
                        <el-radio v-model="dialogForm.style" label="1">横向滚动</el-radio>
                        <el-radio v-model="dialogForm.style" label="2">一行两个</el-radio>
                    </el-form-item>
                    <el-form-item label-width="100px" label="商品数量" prop="goods_num">
                        <el-input :max="20" :min="1" oninput="this.value = this.value.replace(/[^0-9]/g, '');" v-model="dialogForm.goods_num" type="number" placeholder="请输入商品数"></el-input>
                        <div style="color: #C3C3C3;font-size: 12px;">最多限制20件商品</div>
                    </el-form-item>
                </template>
            </el-form>
    ...
    const app = new Vue({
        ...
        data() {
            let min_number = (rule, value, callback) => {
                if (value > 20) {
                    callback(new Error('商品数不得大于20'));
                } if (value < 1) {
                    callback(new Error('商品数需大于0'));
                } else {
                    callback();
                }
            };
            return {
                ...
                dialogRule: {
                    goods_num: [
                        {required: true, message: '请输入商品数', trigger: 'change'},
        ...
        methods: {
                ...
                request({
                    ...
                    if (e.data.code == 0) {
                        ...
                        data.forEach(function (item, index) {
                            self.moduleAdd(item);
                            if(item.key != 'video') {
                                for(let list of self.option) {
                                    for(let index in list.list) {
                                        if(list.list[index].key == item.key && (item.is_edit == 1 || list.name == '插件')) {
                                            if(list.name == '插件' && list.list[index].key != 'mch') {
                                                item.is_edit = 1;
                                                item.goods_num = !item.goods_num ? 10 : item.goods_num;
                                                item.style = !item.style ? '1' : item.style;
                                            }
                                            list.list.splice(index,1,item);
                                        }
                                    }
                                }
                            }
            ...
            videoUrl(e) {
                if (e.length) {
                    ...
                    this.dialogForm.video_url = e[0].url;
                    this.list[this.listIndex].name = e[0].name;
                    this.dialogForm.name = e[0].name;
                    this.$refs.dialogForm.validateField('video_url');
                }
            },
            videoPicUrl(e) {
                if (e.length) {
                    this.dialogForm.video_pic_url = e[0].url;
            ...
            // 编辑
            openDialogForm(item, index) {
                let self = this;
                console.log(item)
                if(index > -1) {
                    self.dialogFormVisible = true;
                    self.listIndex = index;
                    self.dialogForm = JSON.parse(JSON.stringify(item));
                }else {
                    let idx = -1;
                    for(let index in self.list) {
                        if(self.list[index].key == item.key) {
                            idx = index;
                            break;
                        }
                    }
                    self.dialogFormVisible = true;
                    if(idx > 0) {
                        self.dialogForm = JSON.parse(JSON.stringify(self.list[idx]));
                    }else {
                        self.dialogForm = JSON.parse(JSON.stringify(item));
                    }
                }
            ...
            dialogSubmit(formName) {
                ...
                self.$refs[formName].validate((valid) => {
                    if (valid) {
                        let obj = {};
                        obj = JSON.parse(JSON.stringify(self.dialogForm));
                        if(obj.key != 'video') {
                            for(let item of self.option) {
                                if(this.activeName == item.key) {
                                    for(let idx in item.list) {
                                        if(item.list[idx].key == obj.key) {
                                            obj.edited = true;
                                            item.list.splice(idx,1,obj);
                                        }
                                    }
                                }
                            }
                            for(let index in self.list) {
                                if(self.list[index].key == obj.key) {
                                    self.list.splice(index,1,obj);
                                }
                            }
                        }else {
                            self.list[self.listIndex] = obj;
                        }
