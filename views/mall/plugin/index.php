                ...
                <div class="plugin-list" flex="dir:left">
                    <template v-for="(plugin, pluginIndex) in cat.plugins">
                        <template v-if="!isSearching || plugin.inSearch">
                            <!--
                             #modify: 2021-03-25 过滤删除应用
                             is_delete==0
                             -->
                            <div v-if="plugin.is_delete==0" :key="pluginIndex" @click="entryPlugin(plugin)" :class="[{'lock' : plugin.is_delete==1}, 'plugin-item']" flex="dir:left box:first">
                                <div style="z-index: 1">
