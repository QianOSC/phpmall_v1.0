...
<div id="app" v-cloak>
    <el-card shadow="never" style="border:0" body-style="background-color: #f3f3f3;padding: 10px 0 0;">
        ...
        <div class="table-body" v-loading="listLoading">
            ...
            <app-platform v-model="platform" @change='search'></app-platform>
        ...
        <el-dialog title="修改门店" :visible.sync="dialogEdit" width="30%">
            <el-form :model="editForm" label-width="100px" :rules="editFormRules" ref="editForm">
                <el-form-item label="门店选择" prop="name_id">
                    <el-autocomplete
                        style="width: 50%"
                        size="small"
                        v-model="editForm.name_id"
                        value-key="name_id"
                        :fetch-suggestions="storeQuerySearchAsync"
                        placeholder="请输入内容"
                        @select="editStoreClick"> 
                    </el-autocomplete>
                </el-form-item>
                <el-form-item>
    ...
    const app = new Vue({
        ...
        data() {
            ...
            let validatePass2 = (rule, value, callback) => {
                if (!this.addForm.hasOwnProperty('store_id') || !this.addForm.store_id) {
                    callback('请选择门店');
                } else {
                    callback();
                }
        ...
        methods: {
            ...
            storeClick(row) {
                //this.addForm = Object.assign({id: row.id}, this.addForm);
                this.addForm.store_id = row.id;
                this.$refs.addForm.validateField('name_id');
            },

            editStoreClick(row) {
                //this.addForm = Object.assign({id: row.id}, this.addForm);
                this.editForm.store_id = row.id;
                this.$refs.editForm.validateField('name_id');
            },
