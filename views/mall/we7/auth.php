...
<div id="app" v-cloak>
    <el-card shadow="never" style="border:0" body-style="background-color: #f3f3f3;padding: 10px 0 0;">
        ...
        <div class="table-body">
            <el-form style="margin-bottom: 25px;" @submit.native.prevent size="small" inline :model="search">
                <el-form-item>
                ...
                <el-form-item v-if="status && multipleSelection.length < 1">
                    ...
                    <div>
                        ...
                        <div style="display: inline-block;position: relative;">
                            <el-radio v-model="status" label="2">默认自定义权限</el-radio>
                            <div v-if="status == 2" style="position: absolute;left: 25px;">
                                <el-button @click="customEdit" type="primary" size="mini">
    ...
    <el-dialog
        ...
        <div class="window">
            <!-- is_custom_edit 自定义权限编辑时不显示 -->
            <el-checkbox v-if="is_custom_edit" class="check-title" v-model="is_default" :true-label="1" :false-label="0">
                默认权限
    ...
    const app = new Vue({
        ...
        methods: {
            ...
            getList() {
                ...
                request({
                    ...
                    if (e.data.code === 0) {
                        ...
                        self.custom_permissions = e.data.data.custom_permissions;
                    } else {
            ...
            customEdit() {
                let self = this;
                self.dialogVisible = true;
                self.is_custom_edit = false;
                self.is_default = 0;
                self.secondary_permissions = this.custom_permissions.secondary_permissions;
                self.checkedMallPermissions = this.custom_permissions.mall_permissions;
                self.checkedPluginsPermissions = this.custom_permissions.plugin_permissions;
            ...
            updatePermission() {
                ...
                if (self.is_custom_edit) {
                    ...
                    self.updateStatus(self.checkedMallPermissions, self.checkedPluginsPermissions);
                }
            },
            // 搜索
            commonSearch() {
            ...
            // 更新子账户默认权限状态
            updateStatus($mall_permissions = [], $plugin_permissions = []) {
                let self = this;
                self.btnLoading = true;
                request({
                    params: {
                    ...
                    data: {
                        status: self.status,
                        mall_permissions: JSON.stringify($mall_permissions),
                        plugin_permissions: JSON.stringify($plugin_permissions),
                        secondary_permissions: JSON.stringify(self.secondary_permissions)
                    }
                }).then(e => {
                    self.btnLoading = false;
                    if (e.data.code === 0) {
            ...
            customPermissionText() {
                let total = this.permissions.mall.length + this.permissions.plugins.length;
                let own = this.custom_permissions.mall_permissions.length + this.custom_permissions.plugin_permissions.length;
